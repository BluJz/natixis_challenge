...
def preprocesser(df: pd.DataFrame, today=today) -> pd.DataFrame:
    df['Total_Requested_Volume'] = pd.to_numeric(df['Total_Requested_Volume'], errors='coerce')  # Convert to float, invalid parsing will be set as NaN
    ...
    df = df[df['B_Price'] > 0]
    df = df.drop(['Sales_Name','Sales_Initial','Tier','AssumedMaturity','YTWDate','Cusip','SpreadvsBenchmarkMid','GSpreadMid'], axis=1)
    col_clean = ['Instrument','BloomIndustrySector','cdcissuer','Country','MidASWSpread','Rating_Moodys','MidZSpread','MidYTM','Rating_Fitch','Rating_SP','Coupon','Ccy','Country','Classification']
    df = df.dropna(subset=col_clean)
    df = df[pd.notna(df['Country']) & df['Country'].apply(lambda x: isinstance(x, str))]
    df = df[pd.notna(df['BloomIndustrySubGroup']) & df['BloomIndustrySubGroup'].apply(lambda x: isinstance(x, str))]
    df = df[pd.notna(df['cdcissuer']) & df['cdcissuer'].apply(lambda x: isinstance(x, str))]
    # Convert to datetime
