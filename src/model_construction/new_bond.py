...
rating_mapping = {
    ...
    "Caa1": 5,
    "WR": 0,
    "NR": 0,  # WR (Withdrawn Rating) and NR (Not Rated) can be treated as 0 or another scheme
}
...
def new_bond_to_vec(
    ...
    new_bond_encoded_categorical = one_hot_encoder.transform(
        new_bond_df[categorical_features]
    )
    new_bond_features = np.hstack(
        (new_bond_scaled_numerical, new_bond_encoded_categorical.toarray())
    )
