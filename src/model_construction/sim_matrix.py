...
def isin_to_features_func(df_unique, features):
...
def client_apetite(client, df, isin_to_features, recent100=False):
    ...
    for isin, weight in percentage.items():
...
def client_apetite_dict(df, isin_to_features, recent100=False):
    ...
    diction = {
        client: client_apetite(
            client=client, df=df, isin_to_features=isin_to_features, recent100=recent100
        )
        for client in df.company_short_name.unique()
    }
    return diction
